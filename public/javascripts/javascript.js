...
function hocPhanInit(cls) {
	...
	function makeEle(mahp) {
		...
		list.forEach((hp) => {
			...
			id_to_hocs_2[mahp].push(hp.id_to_hoc)
		...
		id_to_hocs = Object.values(id_to_hocs_2)
		updateSlot();
	}
...
function updateSlot() {
	fetch(urlApis.getSlot, {
		...
		data.data.forEach(({ id_to_hoc, sl_cp, sl_dk }) => {
			var el = document.querySelector(`.list-hp-item[id-to-hoc="${id_to_hoc}"] p:nth-child(4)`)
			if (el) el.textContent = `Slots: ${sl_dk}/${sl_cp}`;
		})
	})
}

function updateSlotInterval() {
	if (IntervalId) clearInterval(IntervalId)
	IntervalId = setInterval(updateSlot, 10 * 60 * 1000)
}
