...
async function registerAccount(
  ...
  const sqlCreateUserLoginInfo = `
        INSERT INTO user_login_info(username, pass, email, id, created, type_signup)
        VALUES(?,?,?,?,NOW(),?)
    `;
  ...
  var [err, result, fields] = await query(sqlCreateUserInfo, [
    uuid,
    fullName,
    mssv,
    khao,
  ...
  [err, result, fields] = await query(sqlCreateUserLoginInfo, [
    user,
    password,
    email,
    uuid,
    typeSignup ? typeSignup : "DEFAULT"
  ]);
...
async function getDsTkb(userId) {
  const sql = `SELECT id, tkb_name, description, json_data, thumbnails, date_save FROM tkb_save WHERE JSON_CONTAINS(id_user, '${escape(
    userId
  ).replace(/'/g, '"')}', "$")`;
  const [err, result, fields] = await query(sql);
...
async function getTkb(tkbId) {
  var sql =
    "SELECT id_user, tkb_name, description, json_data, date_save FROM tkb_save " +
    "WHERE id = ?";
  ...
  const sql = `
        ...
        SET tkb_name = CASE 
            ...
            json_data = CASE 
                WHEN ${escape(
    JSON.stringify(idToHocs)
  )} IS NOT NULL THEN  ${escape(JSON.stringify(idToHocs))}
                ELSE  json_data
            END,
            description = CASE 
                WHEN ${escape(description)} IS NOT NULL THEN  ${escape(
    description
  )}
                ELSE  description 
            END,
            thumbnails = CASE 
                ...
                ELSE  thumbnails 
            END
        WHERE id = ${escape(tkbId)} AND JSON_CONTAINS(id_user, '${escape(
    userId
  ).replace(/'/g, '"')}', "$")
    `;
  const [err, result, fields] = await query(sql);
...
async function inviteId2TkbId(inviteId) {
  ...
  if (result[0]) return [err, result[0].tkb_id];
...
async function deleteInviteByTkbId(tkbId) {
  ...
  const [err, result, fields] = await query(
    "DELETE FROM invite_link WHERE tkb_id=?;",
    [tkbId]
  );
  return [err, result];
}
...
async function addUserToTkb(tkbId, userId) {
  const sql = `
    Update tkb_save
    set id_user=CASE 
        WHEN JSON_CONTAINS(id_user, '${escape(userId).replace(
    /'/g,
    '"'
  )}') THEN id_user
